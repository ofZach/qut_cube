...
void ofxPanZoom::update(float deltaTime){
    ...
    //} else{
    //    zoom = ofLerp(zoom, desiredZoom, 1. - pow(smoothFactor, myAppOF->currentMs));
    //}
    //test framerate independent http://joshparnell.com/blog/2012/11/26/stable-exponential-smoothing-under-variable-framerate/
...
void ofxPanZoom::touchMoved(ofTouchEventArgs &touch){
	...
	if (touchIDOrder.size() == 1){
		...
		if (touchIDOrder.size() >= 2){
			...
			if ( touchIDOrder.size() == 2 ){
				p = 0.5 * ( lastTouch[touch.id] - ofVec2f(touch.x,touch.y) );
                //0.5 to average both touch offsets
				passOffset = p;
                desiredOffset += - p * (1.0 / desiredZoom);
                
//                myAppOF->halfWindow += - p * (1.0 / desiredZoom);
			}
